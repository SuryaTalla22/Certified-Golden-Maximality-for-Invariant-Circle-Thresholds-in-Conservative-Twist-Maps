...
from kam_theorem_suite.proof_driver import build_transport_certified_limit_report
        ...
        "approximants": [
            ...
            },
            {
                ...
                "bridge_report": {"crossing_certificate": {
                    ...
                    "branch_summary": {"gprime_interval_lo": 0.9, "gprime_interval_hi": 1.2, "tangent_inclusion_success": True, "branch_tube": {"branch_residual_width": 1e-8, "tube_sup_width": 2e-4, "slope_center_inf": 3.0, "interval_tangent_success": True, "interval_tangent_contraction": 0.18}}}},
            },
            {
                "p": 13,
                "q": 21,
                ...
                "bridge_report": {"crossing_certificate": {
                    ...
                    "branch_summary": {"gprime_interval_lo": 1.0, "gprime_interval_hi": 1.3, "tangent_inclusion_success": True, "branch_tube": {"branch_residual_width": 1e-8, "tube_sup_width": 1e-4, "slope_center_inf": 2.4, "interval_tangent_success": True, "interval_tangent_contraction": 0.15}}}},
            },
            {
                "p": 21,
                "q": 34,
                ...
                "bridge_report": {"crossing_certificate": {
                    ...
                    "branch_summary": {"gprime_interval_lo": 1.1, "gprime_interval_hi": 1.4, "tangent_inclusion_success": True, "branch_tube": {"branch_residual_width": 1e-8, "tube_sup_width": 7e-5, "slope_center_inf": 2.0, "interval_tangent_success": True, "interval_tangent_contraction": 0.12}}}},
            },
        ]
    }


def test_transport_certified_limit_uses_refined_transport_windows() -> None:
    cert = build_transport_certified_limit_certificate(_synthetic_ladder(), min_chain_length=4)
    d = cert.to_dict()
    assert d["theorem_status"].startswith("transport-certified-limit-")
    assert d["chain_length"] >= 4
    assert d["limit_interval_lo"] is not None
    assert d["limit_interval_hi"] is not None
    assert d["transport_intersection_width"] is not None
    assert d["observed_transport_contraction_ratio"] is not None and d["observed_transport_contraction_ratio"] < 1.0
    first = d["entries"][0]
    assert first["transport_window_width"] <= first["raw_window_width"]
    assert d["mean_transport_improvement"] is not None and d["mean_transport_improvement"] >= 0.0


def test_driver_exposes_transport_certified_layer() -> None:
    report = build_transport_certified_limit_report(_synthetic_ladder(), min_chain_length=4)
    assert report["chain_length"] >= 4
    assert report["theorem_status"].startswith("transport-certified-limit-")
