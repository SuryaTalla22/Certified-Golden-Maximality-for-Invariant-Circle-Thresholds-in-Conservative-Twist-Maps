...
from kam_theorem_suite.tail_cauchy_potential_control import build_tail_cauchy_potential_certificate
...
def test_tail_cauchy_potential_builds_classwise_remainder_decomposition() -> None:
    cert = build_tail_cauchy_potential_certificate(_synthetic_ladder(), min_chain_length=5)
    d = cert.to_dict()
    assert d['theorem_status'].startswith('tail-cauchy-potential-')
    assert d['chain_length'] >= 5
    assert d['selected_limit_interval_lo'] is not None
    assert d['selected_limit_interval_hi'] is not None
    assert d['suffix_budget_nonincreasing'] is True
    total = d['full_tail_budget']
    parts = [
        d['full_tail_triple_reinforced_budget'],
        d['full_tail_pair_anchored_budget'],
        d['full_tail_transport_fallback_budget'],
        d['full_tail_geometric_budget'],
    ]
    assert total is not None
    assert abs(sum(parts) - total) < 1e-12
    shares = [
        d['full_tail_triple_reinforced_share'],
        d['full_tail_pair_anchored_share'],
        d['full_tail_transport_fallback_share'],
        d['full_tail_geometric_share'],
    ]
    assert abs(sum(shares) - 1.0) < 1e-9


def test_driver_exposes_tail_cauchy_potential_layer() -> None:
    report = build_tail_cauchy_potential_report(_synthetic_ladder(), min_chain_length=5)
    assert report['chain_length'] >= 5
    assert report['theorem_status'].startswith('tail-cauchy-potential-')
