...
def get_residue_and_derivative_iv(p, q, K_iv, family, x_guess, dps=100, target_residue=0.25):
    """Return enclosures for the crossing function ``g(K)=|R(K)|-target`` and its derivative.

    This function upgrades the original placeholder into a concrete proof-bridge
    routine:

    1. track the periodic branch to the endpoints of ``K_iv`` and build a branch
       tube over the window;
    2. evaluate the Greene residue over that tube using interval arithmetic;
    3. evaluate the total derivative ``dR/dK`` over the same tube;
    4. convert those enclosures into bounds for ``g`` and ``g'``.

    The result is returned as a dictionary so downstream routines can inspect the
    branch-tube diagnostics instead of relying on a single boolean.
    """
    mp.mp.dps = dps
    family = family or HarmonicFamily()
    K_box = iv_scalar(K_iv)
    K_lo = float(K_box.a)
    K_hi = float(K_box.b)

    tube = build_branch_tube(
        p=p,
        q=q,
        K_lo=K_lo,
        K_hi=K_hi,
        family=family,
        x_guess=np.asarray(x_guess, dtype=float) if x_guess is not None else None,
    )

    R_iv = greene_residue_iv(tube.x_iv, p, K_box, family)
    abs_R_iv = abs(R_iv)
    g_iv = abs_R_iv - iv_scalar(float(target_residue))

    dR_iv, tangent_res = residue_derivative_iv(tube.x_iv, p, K_box, family)
    # Differentiate ``|R|`` carefully. When the residue interval has a definite
    # sign we can propagate the exact derivative of the absolute value. When the
    # interval straddles zero we fall back to a conservative magnitude enclosure.
    if float(R_iv.b) < 0.0:
        gp_iv = -dR_iv
    elif float(R_iv.a) > 0.0:
        gp_iv = dR_iv
    else:
        gp_iv = abs(dR_iv)
    ...
    return {
        ...
        "residue_interval_lo": float(R_iv.a),
        "residue_interval_hi": float(R_iv.b),
        "abs_residue_interval_lo": float(abs_R_iv.a),
        "abs_residue_interval_hi": float(abs_R_iv.b),
        "g_interval_lo": float(g_iv.a),
        "g_interval_hi": float(g_iv.b),
        "gprime_interval_lo": float(gp_iv.a),
        "gprime_interval_hi": float(gp_iv.b),
        # Backward-compatible aliases retained for older notebook cells.
        "dg_interval_lo": float(gp_iv.a),
        "dg_interval_hi": float(gp_iv.b),
        "tangent_inclusion_success": bool(tangent_res.success),
        "tangent_contraction_bound": float(tangent_res.contraction_bound),
        "message": tube.message,
    }
...
def _proof_profile_for_row(p: int, q: int) -> dict:
    default = {
        "newton_h_list": (1e-6, 2e-6, 5e-6, 1e-5, 2e-5),
        "newton_derivative_floor": 8e-4,
        "newton_bracket_fractions": (0.125, 0.25, 0.5),
        "micro_h_list": (1e-6, 2e-6, 5e-6, 1e-5, 2e-5),
        "micro_grid_points": 21,
        "micro_max_candidates": 8,
        "symmetric_h_list": (1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5),
        "targeted_center_margin": 2e-4,
        "targeted_per_side_points": 25,
        "dense_per_side_points": 21,
    }

    profiles = {
        (55, 89): {
            **default,
            "newton_h_list": (2e-7, 5e-7, 1e-6, 2e-6, 5e-6),
            "newton_derivative_floor": 1e-3,
            "micro_h_list": (5e-7, 1e-6, 2e-6, 5e-6),
            "micro_grid_points": 17,
            "micro_max_candidates": 6,
            "symmetric_h_list": (2e-6, 5e-6, 1e-5),
            "targeted_center_margin": 1e-4,
        },
        (34, 55): {
            **default,
            "newton_h_list": (5e-7, 1e-6, 2e-6, 5e-6, 1e-5),
            "newton_derivative_floor": 1e-3,
            "micro_h_list": (1e-6, 2e-6, 5e-6, 1e-5),
            "micro_grid_points": 17,
            "micro_max_candidates": 6,
            "symmetric_h_list": (2e-6, 5e-6, 1e-5, 2e-5),
            "targeted_center_margin": 1.5e-4,
        },
        (21, 34): {
            **default,
            "newton_h_list": (2e-6, 5e-6, 1e-5, 2e-5, 5e-5),
            "newton_derivative_floor": 1e-3,
            "micro_h_list": (2e-6, 5e-6, 1e-5, 2e-5),
            "micro_grid_points": 17,
            "micro_max_candidates": 6,
            "symmetric_h_list": (2e-6, 5e-6, 1e-5, 2e-5, 5e-5),
            "targeted_center_margin": 2e-4,
        },
        (29, 70): {
            **default,
            "newton_h_list": (5e-7, 1e-6, 2e-6, 5e-6, 1e-5, 2e-5),
            "newton_derivative_floor": 8e-4,
            "micro_h_list": (1e-6, 2e-6, 5e-6, 1e-5),
            "micro_grid_points": 19,
            "micro_max_candidates": 8,
            "symmetric_h_list": (1e-6, 2e-6, 5e-6, 1e-5, 2e-5),
            "targeted_center_margin": 1.5e-4,
        },
        (18, 43): {
            **default,
            "newton_h_list": (1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5),
            "newton_derivative_floor": 8e-4,
            "micro_h_list": (2e-6, 5e-6, 1e-5, 2e-5),
            "micro_grid_points": 21,
            "micro_max_candidates": 8,
    ...
    return profiles.get((p, q), default)


def _proof_mode_strict_pair(
    p: int,
    q: int,
    K_center: float,
    K_lo: float,
    ...
    family = family or HarmonicFamily()
    profile = _proof_profile_for_row(p, q)
    initial_half_width = max(0.5 * (float(K_hi) - float(K_lo)), 5e-6)

    attempts = []

    def _record(name: str, out: dict):
        attempts.append({
            "method": name,
            "success": bool(out.get("success", False)),
            "message": out.get("message", ""),
        })
        if "attempts" not in out:
            out["attempts"] = list(attempts)
        return out

    out = certify_crossing_via_newton_local(
        p=p,
        q=q,
        K_center=K_center,
        family=family,
        target_residue=target_residue,
        h_list=profile["newton_h_list"],
        derivative_floor=profile["newton_derivative_floor"],
        bracket_fractions=profile["newton_bracket_fractions"],
    )
    if out.get("success", False):
        return _record("certify_crossing_via_newton_local", out)
    _record("certify_crossing_via_newton_local", out)

    out = certify_crossing_via_derivative(
        p=p,
        q=q,
        K_center=K_center,
        family=family,
    ...
    if out.get("success", False):
        return _record("certify_crossing_via_derivative", out)
    _record("certify_crossing_via_derivative", out)

    out = micro_center_scout_pair_search(
        p=p,
        q=q,
        K_center=K_center,
        family=family,
        target_residue=target_residue,
        h_list=profile["micro_h_list"],
        grid_points=profile["micro_grid_points"],
        max_candidates_per_side=profile["micro_max_candidates"],
    )
    if out.get("success", False):
        return _record("micro_center_scout_pair_search", out)
    _record("micro_center_scout_pair_search", out)

    out = symmetric_strict_pair_search(
        p=p,
        q=q,
        K_center=K_center,
        family=family,
        target_residue=target_residue,
        h_list=profile["symmetric_h_list"],
    )
    if out.get("success", False):
        return _record("symmetric_strict_pair_search", out)
    _record("symmetric_strict_pair_search", out)

    out = search_strict_endpoint_pair_targeted(
        p=p,
        q=q,
        K_center=K_center,
        ...
        target_residue=target_residue,
        max_expand=8,
        per_side_points=profile["targeted_per_side_points"],
        center_margin=profile["targeted_center_margin"],
        require_strict_buffer=True,
    )
    if out.get("success", False):
        return _record("search_strict_endpoint_pair_targeted", out)
    _record("search_strict_endpoint_pair_targeted", out)

    out = search_strict_endpoint_pair_dense(
        p=p,
        q=q,
        K_center=K_center,
        initial_half_width=initial_half_width,
        ...
        target_residue=target_residue,
        max_expand=8,
        per_side_points=profile["dense_per_side_points"],
    )
    if out.get("success", False):
        return _record("search_strict_endpoint_pair_dense", out)
    _record("search_strict_endpoint_pair_dense", out)

    return {
        "success": False,
        "proof_ready": False,
        "message": "All theorem-mode strict-pair methods failed.",
        "method": "_proof_mode_strict_pair",
        "attempts": attempts,
    }
...
def certified_unique_crossing_on_window(
    ...
    def _is_plain_standard_family(fam: HarmonicFamily) -> bool:
        hs = getattr(fam, "harmonics", None)
        if hs is None or len(hs) != 1:
            return False
        amp, mode, phase = hs[0]
        return abs(float(amp) - 1.0) < 1e-15 and int(mode) == 1 and abs(float(phase)) < 1e-15
    ...
    GOLDEN_TARGETS = set(GOLDEN_HINTS.keys())
    ...
    # Always pre-refine. The old version skipped this for the golden rows, which is exactly
    # what makes theorem-mode fragile there.
    ...
    if proof_mode:
        # Start from the best localized center we have.
        ...
        # For the three theorem-grade golden rows in the plain standard map, use the
        # already-known asymptotic centers and then refine them before any local proof.
        if (p, q) in GOLDEN_TARGETS and _is_plain_standard_family(family):
            K_center = GOLDEN_HINTS[(p, q)]
            try:
                refined = deep_refine_and_certify(
                    p=p,
                    q=q,
                    K_center=K_center,
                    target_residue=target_residue,
                ...
                if refined.get("success", False):
                    K_center = float(refined["refined_center"])
            except Exception:
                # If deep refinement fails, still fall back to the hint rather than the coarse scan center.
                pass

        # Build a proof-method chain. We only return early if a method is truly proof-ready.
        strict_seed = None
        method_tries = []

        def _try(method_name: str, fn):
            try:
                out = fn()
                ...
                out = {
                    "success": False,
                    "proof_ready": False,
                    "message": f"{method_name} raised {type(e).__name__}: {e}",
                    "method": method_name,
                }
            method_tries.append(out)
            return out

        if (p, q) == (55, 89):
            c1 = _try(
                "certify_crossing_via_newton_local",
                lambda: certify_crossing_via_newton_local(
                    p=p,
                    q=q,
                    K_center=K_center,
                    family=family,
            ...
            if c1.get("proof_ready", False):
                return {
                    "success": True,
                    "proof_ready": True,
                    "certification_tier": c1.get("certification_tier", "strict"),
                    "unique_crossing": bool(c1.get("unique_crossing", True)),
                    "transverse_crossing": bool(c1.get("transverse_crossing", True)),
                    "K_lo": float(c1["K_lo"]),
                    "K_hi": float(c1["K_hi"]),
                    "width": float(c1["width"]),
                    "left": c1["left"],
                    "right": c1["right"],
                    "message": c1.get("message", "Local theorem-style proof succeeded."),
                    "strict_endpoint_search": c1,
                    "proof_method_chain": method_tries,
                    "pre_refine": pre,
                    "scan_summary": scan_rows,
                }
            ...
            c2 = _try(
                ...
                lambda: certify_crossing_via_derivative(
                    p=p,
                    q=q,
                    K_center=K_center,
                    family=family,
            ...
            if c2.get("proof_ready", False):
                return {
                    "success": True,
                    "proof_ready": True,
                    "certification_tier": c2.get("certification_tier", "strict"),
                    "unique_crossing": bool(c2.get("unique_crossing", True)),
                    "transverse_crossing": bool(c2.get("transverse_crossing", True)),
                    "K_lo": float(c2["K_lo"]),
                    "K_hi": float(c2["K_hi"]),
                    "width": float(c2["width"]),
                    "left": c2["left"],
                    "right": c2["right"],
                    "message": c2.get("message", "Derivative-based theorem-style proof succeeded."),
                    "strict_endpoint_search": c2,
                    "proof_method_chain": method_tries,
                    "pre_refine": pre,
                    "scan_summary": scan_rows,
                }
            ...
            # silver_12_29: already close in your run (strict tier reached),
            # so refine the center first, then use broader local theorem checks.
            try:
                refined = deep_refine_and_certify(
                    p=p,
                    q=q,
            ...
            c3 = _try(
                ...
                lambda: search_strict_endpoint_pair_targeted(
                    p=p,
                    q=q,
                    K_center=K_center,
                    initial_half_width=max(0.75 * (float(K_hi) - float(K_lo)), 2e-5),
                    family=family,
                    target_residue=target_residue,
                    max_expand=10,
                    per_side_points=31,
            ...
            if c3.get("proof_ready", False):
                return {
                    "success": True,
                    "proof_ready": True,
                    "certification_tier": c3.get("certification_tier", "strict"),
                    "unique_crossing": bool(c3.get("unique_crossing", True)),
                    "transverse_crossing": bool(c3.get("transverse_crossing", True)),
                    "K_lo": float(c3["K_lo"]),
                    "K_hi": float(c3["K_hi"]),
                    "width": float(c3.get("width", float(c3["K_hi"]) - float(c3["K_lo"]))),
                    "left": c3["left"],
                    "right": c3["right"],
                    "message": c3.get("message", "Targeted strict-pair theorem-style proof succeeded."),
                    "strict_endpoint_search": c3,
                    "proof_method_chain": method_tries,
                    "pre_refine": pre,
                    "scan_summary": scan_rows,
                }

            if c3.get("success", False) and c3.get("left") is not None and c3.get("right") is not None and strict_seed is None:
                strict_seed = c3

        elif (p, q) == (29, 70):
            # silver_29_70: harder challenger, so first refine the center,
            # then use a wider h-list and larger targeted search.
            try:
                refined = deep_refine_and_certify(
                    p=p,
                    q=q,
            ...
            c1 = _try(
                ...
                lambda: certify_crossing_via_newton_local(
                    ...
                    family=family,
                    target_residue=target_residue,
                    h_list=(5e-6, 1e-5, 2e-5, 5e-5, 1e-4, 2e-4),
            ...
            c3 = _try(
                ...
                lambda: search_strict_endpoint_pair_targeted(
                    ...
                    q=q,
                    K_center=K_center,
                    initial_half_width=max(1.0 * (float(K_hi) - float(K_lo)), 5e-5),
                    family=family,
                    target_residue=target_residue,
                    max_expand=12,
                    per_side_points=35,
            ...
            # cf3_10_33: same idea as 29/70, but allow an even broader center search.
            try:
                refined = deep_refine_and_certify(
                    p=p,
                    q=q,
        ...
        # General proof-mode fallback chain.
        for method_name, fn in [
            (
                "micro_center_scout_pair_search",
                lambda: micro_center_scout_pair_search(
                    p=p,
                    q=q,
                    K_center=K_center,
                    initial_half_width=max(0.5 * (float(K_hi) - float(K_lo)), 1e-5),
                    family=family,
                    target_residue=target_residue,
                ),
            ),
            (
                "symmetric_strict_pair_search",
                lambda: symmetric_strict_pair_search(
                    p=p,
                    q=q,
                    K_center=K_center,
                    initial_half_width=max(0.5 * (float(K_hi) - float(K_lo)), 1e-5),
            ...
            (
                "search_strict_endpoint_pair_dense",
                lambda: search_strict_endpoint_pair_dense(
                    p=p,
                    q=q,
                    K_center=K_center,
                    initial_half_width=max(0.5 * (float(K_hi) - float(K_lo)), 1e-5),
            ...
            cand = _try(method_name, fn)

            if cand.get("proof_ready", False):
                return {
                    "success": True,
                    "proof_ready": True,
                    "certification_tier": cand.get("certification_tier", "strict"),
                    "unique_crossing": bool(cand.get("unique_crossing", True)),
                    "transverse_crossing": bool(cand.get("transverse_crossing", True)),
                    "K_lo": float(cand["K_lo"]),
                    "K_hi": float(cand["K_hi"]),
                    "width": float(cand.get("width", float(cand["K_hi"]) - float(cand["K_lo"]))),
                    "left": cand["left"],
                    "right": cand["right"],
                    "message": cand.get("message", "Theorem-style proof succeeded."),
                    "strict_endpoint_search": cand,
                    "proof_method_chain": method_tries,
                    "pre_refine": pre,
                    "scan_summary": scan_rows,
                }

            if cand.get("success", False) and cand.get("left") is not None and cand.get("right") is not None and strict_seed is None:
                strict_seed = cand
        ...
        lo = float(strict_seed["K_lo"])
        hi = float(strict_seed["K_hi"])
        last_left = strict_seed["left"]
        last_right = strict_seed["right"]
...
def bisection_crossing_bracket(
    ...
    return certified_unique_crossing_on_window(
        ...
        pre_refine_rounds=pre_refine_rounds,
    )
