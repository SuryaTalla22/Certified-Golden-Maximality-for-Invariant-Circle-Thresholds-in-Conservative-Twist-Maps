...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Generate Phase 5G-b sign/hash-corrected residual/small-divisor formal component attachment candidate."
    )
    ...
    # Default to the convention that Phase 5G replay identified as correct.
    p.add_argument("--force-sign", type=int, choices=[-1, 1], default=-1)
    ...
    summarize_phase5g(
        ...
        f"{args.out_dir}/phase5gb_compact_report.json",
    )
    print(f"[phase5gb] attachment={summary['attachment_path']}")
    print(f"[phase5gb] summary={args.out_dir}/phase5g_component_summary.json")
    print(f"[phase5gb] compact={args.out_dir}/phase5gb_compact_report.json")
