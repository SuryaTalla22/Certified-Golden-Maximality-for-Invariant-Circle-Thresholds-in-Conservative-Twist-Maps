...
from kam_theorem_suite.proof_driver import build_certified_tail_modulus_report
from kam_theorem_suite.certified_tail_modulus_control import build_certified_tail_modulus_certificate
...
def test_certified_tail_modulus_builds_monotone_start_index_radius_map() -> None:
    cert = build_certified_tail_modulus_certificate(_synthetic_ladder(), min_chain_length=5)
    d = cert.to_dict()
    assert d['theorem_status'].startswith('certified-tail-modulus-')
    assert d['chain_length'] >= 5
    assert d['selected_limit_interval_lo'] is not None
    assert d['selected_limit_interval_hi'] is not None
    assert d['modulus_nonincreasing'] is True
    assert d['classwise_modulus_nonincreasing'] is True
    assert d['first_tail_modulus_radius'] >= d['last_tail_modulus_radius']
    nodes = d['nodes']
    assert nodes[0]['tail_modulus_radius'] >= nodes[-1]['tail_modulus_radius']
    assert nodes[0]['remaining_edge_count'] > nodes[-1]['remaining_edge_count']


def test_driver_exposes_certified_tail_modulus_layer() -> None:
    report = build_certified_tail_modulus_report(_synthetic_ladder(), min_chain_length=5)
    assert report['chain_length'] >= 5
    assert report['theorem_status'].startswith('certified-tail-modulus-')
