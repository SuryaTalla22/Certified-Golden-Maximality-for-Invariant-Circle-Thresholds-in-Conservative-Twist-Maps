#!/usr/bin/env python3
import argparse
from kam_theorem_suite.lower_param.phase6_final_integration import assemble_phase6_final_integration


def main():
    ap = argparse.ArgumentParser(description="Phase 6: assemble final Theorem III Track B lower-anchor artifact and downstream regeneration manifest.")
    ap.add_argument("--certificate", required=True)
    ap.add_argument("--promoted-attachment", required=True)
    ap.add_argument("--phase5e-summary", required=True)
    ...
    ap.add_argument("--downstream-output-root", default="artifacts/proof_audit/final_integration_regenerated")
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--force", action="store_true")
    args = ap.parse_args()
    assemble_phase6_final_integration(
        certificate_path=args.certificate,
        promoted_attachment_path=args.promoted_attachment,
        phase5e_summary_path=args.phase5e_summary,
        ...
        max_z=args.max_z,
        downstream_output_root=args.downstream_output_root,
