...
def _compact_geometry_summary(theorem_viii: Mapping[str, Any]) -> dict[str, Any]:
    """Return a small current-reduction geometry summary.

    Different replay routes expose the same geometry in slightly different
    shapes.  The full Theorem-VIII discharge object may contain a nested
    ``current_reduction_geometry_summary`` mapping, while the lightweight
    Stage-108 replay object exposes scalar fields such as
    ``current_reduction_geometry_status`` and
    ``current_reduction_geometry_min_margin`` at top level.  The compact
    report must accept both shapes, otherwise a valid Stage-108 replay can
    look as if the current reduction geometry were missing.
    """
    ...
    # Stage-108 final objects expose the geometry as top-level scalar fields.
    # Normalize them into the compact-report vocabulary used by the final
    # acceptance checks.
    top_level_map = {
        "status": "current_reduction_geometry_status",
        "minimum_certified_margin": "current_reduction_geometry_min_margin",
        "source": "current_reduction_geometry_source",
        "top_gap_scale": "current_reduction_geometry_top_gap_scale",
        "challenger_upper_bound": "current_reduction_geometry_challenger_upper_bound",
        "exhaustion_upper_bound": "current_reduction_geometry_exhaustion_upper_bound",
        "pending_count": "current_reduction_geometry_pending_count",
        "witness_vs_overlap_margin": "current_reduction_geometry_witness_vs_overlap_margin",
        "witness_width_vs_top_gap_margin": "current_reduction_geometry_witness_width_vs_top_gap_margin",
        "witness_lower_vs_challenger_upper_margin": "current_reduction_geometry_witness_lower_vs_challenger_upper_margin",
    }
    for compact_key, source_key in top_level_map.items():
        if compact_key not in out and source_key in theorem_viii:
            out[compact_key] = theorem_viii.get(source_key)

    # Some baseline shells also store the geometry fields; use them only as a
    # fallback so the discharge-level fields win when both exist.
    shell = theorem_viii.get("baseline_reduction_shell") or theorem_viii.get("theorem_viii_shell")
    if isinstance(shell, Mapping):
        for compact_key, source_key in top_level_map.items():
            if compact_key not in out and source_key in shell:
                out[compact_key] = shell.get(source_key)

    status = out.get("status")
    if "available" not in out:
        out["available"] = bool(status and status != "current-reduction-geometry-missing") or out.get("minimum_certified_margin") is not None

    # Keep only reviewer-facing scalar/status fields.  Avoid copying witness
    # arrays or any large nested payloads.
    keep = {
        "available", "status", "minimum_certified_margin", "source",
        "golden_lower_endpoint", "nongolden_upper_endpoint", "final_endpoint_margin",
        "delta_final", "margin", "endpoint_gap", "top_gap_scale",
        "challenger_upper_bound", "exhaustion_upper_bound", "pending_count",
        "witness_vs_overlap_margin", "witness_width_vs_top_gap_margin",
...
def _record_stage_cache_noload(name: str, *, timings: dict[str, Any], note: str = "large cache artifact not loaded") -> Path:
    """Record a cached stage without deserializing its JSON payload.

    Theorem VIII artifacts can be several GB.  A fresh reconstruction should be
    able to reuse those files without materializing them again merely to build a
    final status summary.  This helper records file metadata and returns the
    path, leaving the full object on disk.
    """
    path = stage_cache_path(name)
    print(f"\nStarting stage: {name}", flush=True)
    if not path.exists():
        raise FileNotFoundError(f"Required cached large stage is missing: {relpath(path)}")
    meta = {
        "source": "cache-noload",
        "seconds": 0.0,
    ...
    timings[name] = meta
    print(f"[cache-noload] {name}: {path.stat().st_size} bytes", flush=True)
    return path


def _run_downstream_replay_for_compact_summary() -> Mapping[str, Any]:
    """Produce the small Stage-108 theorem object used for compact summaries.

    This intentionally uses the same public replay route as the referee-facing
    downstream replay, but skips the frozen archive hash check because a fresh
    regeneration may have byte-different theorem_iii.json/theorem_iv.json while
    still satisfying the theorem-facing replay predicates.
    """
    out_dir = PROJECT_ROOT / "artifacts" / "paper_replay" / "downstream_from_cache"
    logs_dir = STAGE_CACHE_DIR / "live_report_replay_logs"
    out_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stdout_path = logs_dir / "replay_downstream_for_compact_summary.stdout.log"
    stderr_path = logs_dir / "replay_downstream_for_compact_summary.stderr.log"
    cmd = [
        sys.executable,
    ...
    print(f"\n[compact_summary_replay] $ {' '.join(cmd)}", flush=True)
    env = os.environ.copy()
    env["PYTHONPATH"] = str(PROJECT_ROOT) + (os.pathsep + env.get("PYTHONPATH", "") if env.get("PYTHONPATH") else "")
    t0 = time.time()
    ...
    dt = time.time() - t0
    print(f"[compact_summary_replay] exit={rc} elapsed={dt:.2f}s", flush=True)
    if rc != 0:
        raise SystemExit(
            "Compact-summary downstream replay failed. See "
            f"{relpath(stdout_path)} and {relpath(stderr_path)}."
        )
    final_obj = out_dir / "stage108_final_theorem_viii_object.json"
    if not final_obj.exists():
        raise FileNotFoundError(f"Expected compact replay object was not written: {relpath(final_obj)}")
    return load_json(final_obj)


def _large_theorem_viii_cache_available() -> bool:
    return stage_cache_path("theorem_viii_base").exists() and stage_cache_path("theorem_viii_discharge").exists()
...
def build_downstream_from_stage_cache(*, live_kwargs: dict[str, Any], use_cache: bool, force_rebuild: bool, timings: dict[str, Any]) -> dict[str, Any]:
    ...
    # Critical memory guard: when Theorem VIII is already cached, do not
    # deserialize the multi-GB theorem_viii_base/theorem_viii_discharge JSON
    # files merely to build the final status report.  Reuse the files on disk
    # and obtain the theorem-facing acceptance fields from the compact Stage-108
    # downstream replay object instead.
    if use_cache and not force_rebuild and _large_theorem_viii_cache_available():
        _record_stage_cache_noload("theorem_viii_base", timings=timings)
        _record_stage_cache_noload("theorem_viii_discharge", timings=timings)
        t0 = time.time()
        print("\nStarting stage: live_theorem_program_discharge_report_summary_cache", flush=True)
        replay_theorem_viii = _run_downstream_replay_for_compact_summary()
        live_report = build_compact_live_theorem_program_report(
            ...
            theorem_viii=replay_theorem_viii,
        )
        timings["live_theorem_program_discharge_report_summary_cache"] = {
            "source": "compact-stage108-replay-summary",
            "seconds": time.time() - t0,
            "cache_path": relpath(stage_cache_path("live_theorem_program_discharge_report_summary_cache")),
            "note": "Built compact report without loading cached multi-GB Theorem VIII artifacts.",
        }
        print(f"[compact-summary] live_theorem_program_discharge_report_summary_cache: {time.time() - t0:.2f}s", flush=True)
        return live_report
    ...
    # First-run memory guard: after theorem_viii_base and theorem_viii_discharge
    # have been written to stage_cache, do not keep the multi-GB Python objects
    # alive merely to assemble the final report.  Drop them and obtain the small
    # theorem-facing status object from downstream replay.  This makes first-run
    # behavior match resume behavior: large Theorem VIII artifacts live on disk,
    # while the live report stores only compact acceptance fields.
    try:
        del theorem_viii_base
    except NameError:
        pass
    try:
        del theorem_viii
    ...
    timings["live_theorem_program_discharge_report_summary_cache"] = {
        "source": "built-then-compact-stage108-replay-summary",
        "seconds": time.time() - t0,
        "cache_path": relpath(stage_cache_path("live_theorem_program_discharge_report_summary_cache")),
        "note": "After writing Theorem VIII artifacts, large in-memory objects are released and the compact report is assembled from downstream replay.",
    }
    print(f"[built-then-compact-summary] live_theorem_program_discharge_report_summary_cache: {time.time() - t0:.2f}s", flush=True)
    return live_report
